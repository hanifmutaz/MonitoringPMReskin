...
| DataTable | **Built, not yet wired (Phase 5)** | `components/data-display/DataTable.jsx` | Presentational table: renders columns/rows against the verified envelope + loading/refreshing/empty/error states | None — receives columns/rows via props, all state copy caller-supplied | `columns, rows, getRowKey, isLoading, isRefreshing, isError, errorState, emptyState, page, limit, total, onPageChange, skeletonRows` | loading (skeleton rows) / refreshing (dimmed, rows stay visible) / populated / empty / error — no-result is caller-composed via the exported `DataTableNoResult` helper, since only the page knows whether zero rows means "no data" or "filters matched nothing" | None yet | Verified via a temporary smoke harness against the representative PM Part envelope (build+lint clean, harness removed after). Sorting/column-filter callbacks intentionally **not** included — no page's current hand-rolled table sorts columns, so that scope would be speculative; add when a real page needs it. Wiring into `PmPartMonitoringPage`/Dashboard is Phase 6/7, not this phase. |
...
| `pm-line/` (brief says "monthly-pm"/"weekly-pm" — codebase term kept, see `ROUTE-MAP.md` note) | New (verified extraction-ready, 24 Aug 2026) | Extract from the three `PmLine*Page` files. Single component to move: `PmLineHistoryForm.jsx` — confirmed single-domain (only consumers are the three `PmLine*Page` files), same precondition Phase 7 checked before moving `WearRing`/`StatusFilterPills`. See `MIGRATION-PLAN.md` Phase 8 for the full line-by-line read-through (bulk-delete complexity in `PmLineHistoryPage`, `PmLineFormPage`'s intentional redundancy with `PmLineStatusPage`'s modal). |
