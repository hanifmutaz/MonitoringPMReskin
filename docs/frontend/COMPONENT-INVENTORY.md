...
| DataTable | **Wired into PmPartMonitoringPage (Phase 7), PmLineStatusPage/PmLineHistoryPage (Phase 8), LinesTab/PartsTab (Phase 9, in progress)** | `components/data-display/DataTable.jsx` | Presentational table: renders columns/rows against the verified envelope + loading/refreshing/empty/error states | None — receives columns/rows via props, all state copy caller-supplied | `columns, rows, getRowKey, isLoading, isRefreshing, isError, errorState, emptyState, page, limit, total, onPageChange, skeletonRows, selection` | loading (skeleton rows, accounts for the selection column too when present) / refreshing (dimmed, rows stay visible) / populated / empty / error — no-result is caller-composed via the exported `DataTableNoResult` helper, since only the page knows whether zero rows means "no data" or "filters matched nothing" | `selection` (added Phase 8, opt-in, shape matches `useRowSelection`'s return value) | Confirmed via 5 live consumers + clean `npm run build`/`eslint`. `page/limit/total/onPageChange` are all optional as a set — `PmLineStatusPage` omits them entirely since its API returns a flat unpaginated array (`usePmLineStatus`, no query params on `GET /pm-line`), and `DataTable` correctly skips rendering `Pagination` when they're absent. `LinesTab` (Phase 9) is the first *client-side*-paginated consumer — passes `filtered.length` as `total` and slices `paged` itself. `PartsTab` (Phase 9) is server-side paginated with `SelectAllAcrossPagesBar` composed above it, same pattern as Phase 7/8's pages. Sorting/column-filter callbacks still intentionally **not** included — still no page needs them. |
