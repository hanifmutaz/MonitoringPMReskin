...
- **Status (verified against codebase, 24 Aug 2026)**: visual reskin already done (`PmPartMonitoringPage.jsx` "batch 4/N", `PmPartFormPage.jsx`/`PmPartHistoryPage.jsx` "batch susulan") — no live legacy classes, table hand-rolled and left as-is per Phase 6.5 decision. **NOT done**: `extract domain/pm-part/` — no such folder exists anywhere in `frontend/src` (confirmed via `find`). This is a real gap, distinct from the DataTable question — it's a code-organization task (pull PM-Part-specific hooks/logic out of `pages/` into a `domain/` folder), not a visual one, and the reskin passes never touched it. Needs an explicit decision: worth doing on its own merits (discoverability, reuse) independent of the DataTable question above.
  - **Open Question #4 (Part Detail) partially resolved**: `pmPartApi.js` has a working `getPmPartDetail(partId)` calling `GET /pm-part/:partId` (backend endpoint fully implemented, returns computed metrics + recent history), but it is **not called from anywhere in the frontend** — confirmed via grep, zero call sites. `PmPartMonitoringPage.jsx`'s two `<Modal>` usages are for "Ganti Part" (replacement action) and "Input Penggantian Part" (form), not a detail view. So: Detail does not exist as route, drawer, or modal today for PM Part — it would be new construction, not a migration, if kept in scope.
...
- **Status (verified against codebase, 24 Aug 2026)**: visual reskin already done across all three files (`PmLineStatusPage.jsx` "batch 2/N", `PmLineFormPage.jsx` "batch 3/N", `PmLineHistoryPage.jsx` "batch 3/N") — no live legacy classes, hand-rolled table left as-is per Phase 6.5.
  - **Open Question #3 resolved**: Monthly and Weekly are **not** a mode/param split across the three `PmLine*Page` files — `PmLineStatusPage.jsx` renders both cycles' columns (`tgl_pm_monthly_terakhir`/`akumulasi_poin_monthly`/`sisa_hari_monthly`/`status_monthly` AND the `_weekly` equivalents) side-by-side in the same row, same table, same page. There is no separate "Weekly mode" of this page. This directly informs `scheduling/` component design (previously blocked): a `ScheduleCalendar`/`Checklist` component, if built, needs to represent one Line's two cycles simultaneously, not switch between them.
